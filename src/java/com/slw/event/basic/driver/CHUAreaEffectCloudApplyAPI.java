package com.slw.event.basic.driver;
...
@api
public class CHUAreaEffectCloudApplyAPI extends AbstractEvent{
    ...
    public Map<String, Construct> evaluate(BindableEvent bindableEvent) throws EventException {
        ...
        if(bindableEvent instanceof CHUAreaEffectCloudApplyInterface){

            CHUAreaEffectCloudApplyInterface e = (CHUAreaEffectCloudApplyInterface) bindableEvent;
            Map<String, Construct> retv = new HashMap<String, Construct>();

            CArray entities = new CArray(Target.UNKNOWN);

            for(LivingEntity entity : e.getEntities()) entities.push(Construct.GetConstruct(entity.getUniqueId()), Target.UNKNOWN);
